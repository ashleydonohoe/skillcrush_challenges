if 1 + 1 == 2
	puts "1 and 1 does indeed equal 2"
end

my_name = "Ashley"
if my_name == "Gabriele"
	puts "Hello, Gabriele!"
else 
	puts "Oops, I thought your name was Gabriele. Sorry about that, #{my_name}!"
end

fav_color = "pink"
if (fav_color == "red")
	puts "Red like fire!"
elsif (fav_color == "orange")
	puts "Orange like, well... an orange!"
elsif (fav_color == "yellow")
	puts "Yellow daffodils are so pretty in the spring!"
elsif (fav_color == "green")
	puts "Have you been to the Emerald City in Oz?"
elsif (fav_color == "blue")
	puts "Blue like the sky!"
elsif (fav_color == "purple")
	puts "Purple plums are the tastiest."
else 
	puts "Hmm, well I don't know what that color is!"
end

#case statements

weather = "rainy"

case ("rainy")
when "sunny" then puts "Let's go to the beach!"
when "snowy" then puts "Let's go sledding!"
when "rainy" then puts "Let's stay home and watch a movie!"
when "stormy" then puts "Stay inside where it's safe!"
end